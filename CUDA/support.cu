...
// __device__ inline
// void encode(size_t* mDIdx, size_t* oneDIdx, size_t * d_m, size_t * d_k){
//         size_t result = 0;
//         for (size_t i = 0; i < *d_m; ++i){
//                result += mDIdx[i] * ipow(*d_k, *d_m - 1 - i);     // can be optimized once set up a reference table for ipow(d_k,id_)
//         }
...
// __device__ inline
// void depleteStorage( size_t* mDarray,  size_t d_amount, size_t * d_m){
//         size_t buffer = 0;
//         size_t i = 0;
//         while(!d_amount && i < *d_m){
//                 if ( !mDarray[i]){
//                         ++i;
//                         continue;
//                 }
